...
export default function Profile() {
  ...
  const loadProfile = async () => {
    ...
    const data = await firebaseClient.entities.UserProfile.filter({ id });
    if (data[0]) {
      ...
      firebaseClient.entities.Post.filter({ author_email: data[0].user_email }, '-created_date', 20).then(setPosts);
      firebaseClient.entities.Listing.filter({ host_email: data[0].user_email }).then(setListings);
    }
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await firebaseClient.entities.UserProfile.update(viewProfile.id, payload);
  ...
  if (!currentUser) return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="text-center">
        ...
        <button onClick={() => firebaseClient.auth.redirectToLogin()} className="bg-primary text-primary-foreground px-6 py-2 rounded-lg text-sm font-semibold">Sign In</button>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto pb-8">
      ...
      <div className="mx-6 mt-6 bg-amber-50 border border-amber-200 rounded-2xl p-5 shadow-sm">
          ...
          <div>
                ...
                <img src={viewProfile.id_document_url} alt="ID" className="w-full rounded-xl border border-border object-cover max-h-40 cursor-zoom-in" onClick={() => window.open(viewProfile.id_document_url, '_blank', 'noopener,noreferrer')} />
              </div>
          }
            {viewProfile.selfie_url &&
          <div>
                <p className="text-xs font-semibold text-muted-foreground mb-1">🤳 Selfie with ID</p>
                <img src={viewProfile.selfie_url} alt="Selfie" className="w-full rounded-xl border border-border object-cover max-h-40 cursor-zoom-in" onClick={() => window.open(viewProfile.selfie_url, '_blank', 'noopener,noreferrer')} />
              </div>
          }
            {viewProfile.face_selfie_url &&
          <div>
                <p className="text-xs font-semibold text-muted-foreground mb-1">📸 Face Check Selfie</p>
                <img src={viewProfile.face_selfie_url} alt="Face selfie" className="w-full rounded-xl border border-border object-cover max-h-40 cursor-zoom-in" onClick={() => window.open(viewProfile.face_selfie_url, '_blank', 'noopener,noreferrer')} />
              </div>
          }
          </div>
      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm" onMouseDown={e => { if (e.target === e.currentTarget) setShowDeleteConfirm(false); }} onTouchEnd={e => { if (e.target === e.currentTarget) setShowDeleteConfirm(false); }}>
          <div className="bg-card rounded-t-3xl sm:rounded-2xl w-full sm:max-w-sm p-6 shadow-xl border border-border">
            ...
            <div className="flex flex-col gap-2">
              <button
              onClick={async () => {
                await firebaseClient.entities.UserProfile.delete(viewProfile.id);
