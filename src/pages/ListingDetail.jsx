...
export default function ListingDetail() {
  ...
  useEffect(() => {
    firebaseClient.entities.Listing.filter({ id }).then(data => {
      if (data[0]) {
        setListing(data[0]);
        firebaseClient.entities.UserProfile.filter({ user_email: data[0].host_email }).then(h => {
          if (h[0]) setHost(h[0]);
        });
      }
    });
    // Load reviews for this listing
    firebaseClient.entities.Review.filter({ listing_id: id }).then(data => {
  ...
  const handleBooking = async () => {
    ...
    try {
      await firebaseClient.entities.Booking.create({
