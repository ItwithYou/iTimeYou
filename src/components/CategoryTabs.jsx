...
export default function CategoryTabs({ activeType, activeCat, onSelectCat, lang }) {
  ...
  return (
    <div className="w-full mb-6 px-2 sm:px-4">
      <div className="bg-card/40 backdrop-blur-xl border border-border/50 shadow-lg shadow-black/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 mx-auto max-w-4xl relative overflow-hidden">
        ...
        <div className="flex flex-wrap justify-center gap-6 sm:gap-10 relative z-10">
          {categories.map(cat => {
            ...
            return (
              ...
              >
                <div className={`w-[52px] h-[52px] rounded-full flex items-center justify-center mb-2 transition-all duration-300 relative ${
                  isActive 
                ...
                <span className={`text-[11px] font-semibold tracking-wide whitespace-nowrap transition-all duration-300 ${
                ...
                {isActive && (
                  <div className="h-1 w-1 bg-primary rounded-full mt-1.5 absolute -bottom-2" />
                )}
              </button>
            );
          })}
